...
var root_path = path.resolve(PROTO_URL)
...
function crawler(proj) {
    ...
    */
    function initSitemap(proj) {
        doc_url = path.resolve(proj, 'data', 'document.js')
...
*/
    function getPageData(page) {
        doc_url = path.resolve(proj, 'files', page.page_original, 'data.js')
    ...
    function parseAnnotation(_data) {
        _data.page.annotations.forEach(an => {
            $ = cheerio.load(an["说明"], {
            ...
            $("p").each((i, p) => {
                if (i == 0) {
                    an.interactName = $(p).text().substring(3)
                    an.interactId = $(p).text().substring(0, 3)
                } else {
                    an.interactDes += $(p).text()
                }
            })
        })
    }
    ...
    function initInteract(proj) {
        pages.forEach(page => {
            var page_url = path.resolve(proj, page.url);
