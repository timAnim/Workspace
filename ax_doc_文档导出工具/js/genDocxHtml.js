...
const LOGO_PATH = path.join(__dirname, '../image/xbrother_logo.png')
...
function genDocx(_src_path) {
    ...
    return crawler(SRC_PATH)
        .then(proj => {
            ...
            let filename = `${proj.proj_info.proj_name}_PRD.html`
            filename = filename.replace(/\(|\)|\s/ig, '_')
            DIST_PATH = path.join(SRC_PATH, '..', filename)

            createHtml(proj)
            var cmd = "start " + DIST_PATH
...
function createHtml({ pages, proj_info, revision_arr, intro, aim_scope, logical_concept_arr, nfr_arr }) {
    ...
    pages.forEach(page => {
        ...
        //         addH4("说明" + note.fn + note.label)
        //         html = cheerio.load("<div></div>", {
        //             decodeEntities: false
        //         })
        //         html("div").append(note["说明"])
        //         html("div p").each((id, ele) => {
        //             txt = html(ele).text()
        //             if (txt) addPara(txt)
        //         })
    ...
    function addCover() {
        ...
        addPara(proj_info.proj_name + '需求规格说明书')
