...
$axure.internal(function($ax) {
    ...
    var _attachDefaultObjectEvent = function(elementIdQuery, elementId, eventName, fn) {
        var func = function() {
            // added by tim
            if (!window.hover_locker && eventName == "click") return dropperClickHdl.apply(this, arguments)
                // added by tim
    ...
    (function() {
        ...
        $ax.event.initKeyEvents = function($query) {
            $query.keydown(function(e) {
                ...
                $ax.setjBrowserEvent(e);
                if (!elementId) fireEventThroughContainers('onKeyDown', undefined, false, [$ax.constants.PAGE_TYPE, $ax.constants.REFERENCE_DIAGRAM_OBJECT_TYPE, $ax.constants.DYNAMIC_PANEL_TYPE, $ax.constants.REPEATER], [$ax.constants.PAGE_TYPE, $ax.constants.REFERENCE_DIAGRAM_OBJECT_TYPE, $ax.constants.LAYER_TYPE]);
            ...
            $query.keyup(function(e) {
                ...
                if (!elementId) fireEventThroughContainers('onKeyUp', undefined, false, [$ax.constants.PAGE_TYPE, $ax.constants.REFERENCE_DIAGRAM_OBJECT_TYPE, $ax.constants.DYNAMIC_PANEL_TYPE, $ax.constants.REPEATER], [$ax.constants.PAGE_TYPE, $ax.constants.REFERENCE_DIAGRAM_OBJECT_TYPE, $ax.constants.LAYER_TYPE]);
    ...
    var _loadDynamicPanelsAndMasters = function(objects, path, itemId) {
        fireEventThroughContainers('onLoad', objects, true, [$ax.constants.PAGE_TYPE, $ax.constants.REFERENCE_DIAGRAM_OBJECT_TYPE, $ax.constants.DYNAMIC_PANEL_TYPE], [$ax.constants.ALL_TYPE], path, itemId);
    };
    $ax.loadDynamicPanelsAndMasters = _loadDynamicPanelsAndMasters;

    var _viewChangePageAndMasters = function(forceSwitchTo) {
        fireEventThroughContainers('onAdaptiveViewChange', undefined, true, [$ax.constants.PAGE_TYPE, $ax.constants.REFERENCE_DIAGRAM_OBJECT_TYPE, $ax.constants.DYNAMIC_PANEL_TYPE], [$ax.constants.PAGE_TYPE, $ax.constants.REFERENCE_DIAGRAM_OBJECT_TYPE]);
    ...
    var _postResize = $ax.postResize = function(e) {
        $ax.setjBrowserEvent(e);
        return fireEventThroughContainers('onResize', undefined, false, [$ax.constants.PAGE_TYPE, $ax.constants.REFERENCE_DIAGRAM_OBJECT_TYPE, $ax.constants.DYNAMIC_PANEL_TYPE, $ax.constants.REPEATER], [$ax.constants.PAGE_TYPE, $ax.constants.REFERENCE_DIAGRAM_OBJECT_TYPE]);
    };
    ...
    var _pageLoad = function() {
        ...
        for (key in PAGE_AXURE_TO_JQUERY_EVENT_NAMES) {
            ...
            (function(axureName) {
                ...
                $(jObj)[actionName](function(e) {
                    $ax.setjBrowserEvent(e);
                    return fireEventThroughContainers(axureName, undefined, false, [$ax.constants.PAGE_TYPE, $ax.constants.REFERENCE_DIAGRAM_OBJECT_TYPE, $ax.constants.DYNAMIC_PANEL_TYPE, $ax.constants.REPEATER], [$ax.constants.PAGE_TYPE, $ax.constants.REFERENCE_DIAGRAM_OBJECT_TYPE]);
                });
            })(key);
        }
        ...
        // 此方法为父frame 调用
        window.closeHoverFn = function() {
            hideAllPanel()
            window.hover_locker = true
        }

        var curDom
        $win.bind($ax.features.eventNames.mouseMoveName, function(e) {
            // 如果悬浮功能不开，不用继续
            if (hover_locker) return hideAllPanel()
            var dom = e.target
            if (dom == curDom) return false
            dom = getOuter(dom)
            curDom = dom

            if (!curDom) {
                $(".dropper-hover").hide(50)
                $(".dropper-ruler").hide(50)
                return false
            }

            $(".dropper-hover").show()
            $(".dropper-ruler").show()
            var axObj = axobj_list[dom.id]

            // 处理当前元素的属性显示
            $('.dropper-hover').css({
                "width": Math.floor(axObj.w),
                "height": Math.floor(axObj.h),
                "top": Math.floor(axObj.y),
                "left": Math.floor(axObj.x),
            })

            $('.dropper-hover').attr({
                "ax-width": Math.floor(axObj.w),
                "ax-height": Math.floor(axObj.h),
            })

            // 父节点的属性显示
            $('.dropper-ruler').css({
                "width": Math.floor(axObj.x),
                "height": Math.floor(axObj.y),
            })
            $('.dropper-ruler').attr({
                "ax-top": Math.floor(axObj.x),
                "ax-left": Math.floor(axObj.y),
            })
            return false
        })


        $win.bind('click', dropperClickHdl)

        // 打开标注的快捷键
        $win.bind("keydown", function(e) {
            if (e.ctrlKey && e.keyCode == 192) {
                window.hover_locker = !window.hover_locker
                window.parent.document.getElementById("dropperBtnImg").setAttribute("lock", window.hover_locker)
                if (window.hover_locker) hideAllPanel()
            }
        })
    }
    ...
    function dropperClickHdl(e) {
        if (hover_locker) return closeHoverFn()
            // 属性窗关闭
        if ($(e.target).hasClass("pin-close")) {
            $(pin_dom).removeClass("pin-dom")
            parent_dom = pin_dom = null
            $(".pin-panel").hide(50)
            $(".parent-panel").hide(50)
            $(".parent-ruler").hide(50)
            return false
        }
        // 获取ax-dom
        var dom = getOuter(e.target)
        if (!dom) return false

        // 获取图形上的父节点
        var axobj = axobj_list[dom.id]
        var parentList = getDiagramParent(axobj)
        var parentobj = null
        if (parentList.length) parentobj = parentList[0]

        // 点击相同元素进行切换显示隐藏，不继续
        if (dom == pin_dom && $(pin_dom).is(':visible')) {
            $(pin_dom).removeClass("pin-dom")
            $(".pin-panel").hide(50)
            $(".parent-panel").hide(50)
            $(".parent-ruler").hide(50)
            ...
            $(pin_dom).addClass("pin-dom")
            $(".pin-panel").show(50)
            $(".parent-panel").show(50)
            $(".parent-ruler").show(50)
            return false
        }
        ...
        pin_dom = dom
        $(pin_dom).addClass("pin-dom")
        initParamPanel(pin_dom, axobj)

        // 初始化父节点面板
        if (parentobj) {
            initParentPanel(parentobj, axobj)
            $(".parent-panel").show(50)
            $(".parent-ruler").show(50)
        } else {
            $(".parent-panel").hide(50)
    ...
    function getAllDomInfo(root_id) {
        const all_dom = $(`#${root_id}>.ax_default, #${root_id}>.panel_state, #${root_id}>.panel_state_content`)
        for (const i in all_dom) {
            if (all_dom[i].id) {
                all_obj[all_dom[i].id] = {
                    x: $(all_dom[i]).position().left + all_obj[root_id].x || all_obj[root_id].x,
                    y: $(all_dom[i]).position().top + all_obj[root_id].y || all_obj[root_id].y,
                    h: $(all_dom[i]).height() || 0,
                    w: $(all_dom[i]).width() || 0
                }
                getAllDomInfo(all_dom[i].id)
            }
    ...
    // 找到ax的元素
    function getOuter(dom) {
        while (true) {
            if (dom.classList.contains('ax_default')) return dom
            if (dom.tagName == "BODY" || dom.tagName == "HTML") return false;
            dom = dom.parentNode
            getOuter(dom)
        }
    }
    ...
    function hideAllPanel() {
        ...
        $(".parent-panel").hide()
        $(".parent-ruler").hide()
        $(".pin-dom").removeClass("pin-dom")
        pin_dom = null
    }

    function initAllPanel() {
        var el = document.createElement("div")
        el.classList.add("dropper-hover")
        document.body.appendChild(el)

        var ruler = document.createElement("div")
        ruler.classList.add("dropper-ruler")
        document.body.appendChild(ruler)

        var pin_panel = document.createElement("div")
        pin_panel.classList.add("pin-panel")
        document.body.appendChild(pin_panel)

        var parent_panel = document.createElement("div")
        parent_panel.classList.add("parent-panel")
        document.body.appendChild(parent_panel)

        var parent_ruler = document.createElement("div")
        parent_ruler.classList.add("parent-ruler")
        document.body.appendChild(parent_ruler)

        let locker = parent.window.document.getElementById("dropperBtnImg").getAttribute('lock')
        window.hover_locker = (locker == "true")

        parent.window.axobj_list = window.axobj_list = getAllDomInfo("base")
    }

    // 找到图形上的父节点（根据位置算的）
    function getDiagramParent(axObj) {
        var parentResult = []
        for (const k in axobj_list) {
            const obj = axobj_list[k];
            if (obj == axObj) continue
            if (obj.x > axObj.x) continue
            if (obj.y > axObj.y) continue
            if ((obj.x + obj.w) < (axObj.x + axObj.w)) continue
            if ((obj.y + obj.h) < (axObj.y + axObj.h)) continue
            parentResult.push(obj)
        }

        if (parentResult.length > 1) {
            return parentResult.sort((a, b) => a.w * a.h - b.w * b.h)
        } else {
            return parentResult
        }
    }
    ...
    // 获取所有属性并显示
    function initParamPanel(ax_dom, axobj) {
        var stylesDescription = new Map()
        let div_stl = $('#' + ax_dom.id + '_div')
        let img_stl = $('#' + ax_dom.id + '_img')
        let txt_stl = $('#' + ax_dom.id + '_text')
        let inp_stl = $('#' + ax_dom.id + '_input')

        if (div_stl && div_stl.is(':visible')) {
            stylesDescription.set("背景样式", '----')
            stylesDescription.set("颜色", div_stl.css("background-color"))
            stylesDescription.set("填充", div_stl.css("background-image"))
            stylesDescription.set("圆角", div_stl.css("border-radius"))
            stylesDescription.set("阴影", div_stl.css("box-shadow"))
            stylesDescription.set("边框样式", div_stl.css("border-style"))
            stylesDescription.set("边框粗细", div_stl.css("border-width"))
        }
        if (txt_stl && txt_stl.is(':visible')) {
            stylesDescription.set("文本样式", '----')
            stylesDescription.set("颜色", txt_stl.css("color"))
            stylesDescription.set("大小", txt_stl.css("font-size"))
            stylesDescription.set("字体", txt_stl.css("font-family"))
            stylesDescription.set("粗细", txt_stl.css("font-weight"))
            stylesDescription.set("内边距", txt_stl.css("padding"))
        }

        if (inp_stl && inp_stl.is(':visible')) {
            stylesDescription.set("文本样式", '----')
            stylesDescription.set("颜色", inp_stl.css("color"))
            stylesDescription.set("大小", inp_stl.css("font-size"))
            stylesDescription.set("字体", inp_stl.css("font-family"))
            stylesDescription.set("粗细", inp_stl.css("font-weight"))
            stylesDescription.set("内边距", inp_stl.css("padding"))
        }

        updatePinPanel(axobj)

        // 如果有图片则 加载完图片再更新
        if (img_stl && img_stl.is(':visible')) {
            var $img = $("<div></div>"),
                _path,
                fill,
                stroke
            var ext = img_stl.attr('src').split('.')
            if (ext[ext.length - 1] != "svg") return false
            $img.load(img_stl.attr('src'), function() {
                    _path = $img[0].getElementsByTagName('path')[0]

                    if (!_path) return false
                    stroke = _path.getAttribute('stroke')
                    fill = _path.getAttribute('fill')
                    stylesDescription.set("SVG样式", '----')

                    if (stroke && /^#/.test(stroke)) stylesDescription.set("描边", stroke)
                    if (fill && /^#/.test(fill)) stylesDescription.set("填充", fill)
                    if (stroke && /^url/.test(stroke)) {
                        var id = stroke.split("#")[1].replace(')', "")
                        getGradient(stylesDescription, $img.getElementById(id), "描边渐变")
                    }
                    if (fill && /^url/.test(fill)) {
                        var id = stroke.split("#")[1].replace(')', "")
                        getGradient(stylesDescription, $img.getElementById(id), "填充渐变")
                    }
                    updatePinPanel(axobj)
                })
                // 如果没有图片则 直接更新
        ...
        function updatePinPanel(axobj) {
            $(".pin-panel").html('<span class="pin-close">×</span>')
            if (!stylesDescription.size) return false
            var item_k, item_v, item_li
            stylesDescription.forEach((v, k) => {
                // 如果没有值不显示
                if (!v || v == "0px" || v == "none" || v == "null") return false;
                item_li = document.createElement("p")
                item_k = document.createElement("span")
                item_v = document.createElement("span")

                if (/^(rgb|RGB)/.test(v) || hexColorReg.test(v)) {
                    v = colorHex(v)
                    item_v.classList.add('pin-color')
                    item_v.style = '--pin-color:' + v
                }
                ...
                v = v.replace(/\"/g, "")
                if (v == "----") {
                    item_k.classList.add('pin-th')
                    v = ""
                }
                item_k.innerText = k
                item_v.innerText = v
                item_li.appendChild(item_k)
                item_li.appendChild(item_v)
                $(".pin-panel").append(item_li)
            })
    ...
    // 获取svg 中的渐变
    function getGradient(gradientResult, dom, label) {
        var list = dom.getElementsByTagName('stop')
        gradientResult.set(label, "")
        for (let i = 0; i < list.length; i++) {
            gradientResult.set("点" + (i + 1) + "颜色", list[i].getAttribute('stop-color'))
            gradientResult.set("点" + (i + 1) + "位置", list[i].getAttribute('offset'))
        }
    }

    // 初始化父节点高亮的面板
    function initParentPanel(parentobj, axobj) {
        $('.parent-panel').css({
            "top": parentobj.y,
            "left": parentobj.x,
            "width": parentobj.w,
            "height": parentobj.h,
            "--ruler-w": axobj.x - parentobj.x + 'px',
            "--ruler-h": axobj.y - parentobj.y + 'px',
            "--ruler-y": axobj.y - parentobj.y + axobj.h / 2 + 'px',
            "--ruler-x": axobj.x - parentobj.x + axobj.w / 2 + 'px',
        })

        $('.parent-ruler').css({
            "top": parentobj.y,
            "left": parentobj.x,
            "width": axobj.x - parentobj.x + axobj.w / 2,
            "height": axobj.y - parentobj.y + axobj.h / 2,
            "--ruler-y": (axobj.y - parentobj.y) / 2 + 'px',
            "--ruler-x": (axobj.x - parentobj.x) / 2 + 'px',
        })

        $('.parent-ruler').attr({
            "parent-top": axobj.y - parentobj.y,
            "parent-left": axobj.x - parentobj.x,
        })
    }
