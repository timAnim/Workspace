...
class Rtdb(object):
    def __init__(self, host='localhost', port=6379, username='', password=''):
...
if __name__ == "__main__":
    ...
    r = Rtdb()
    val = ''
    point_id = '0_5259_1_10003_0'
    if len(sys.argv) > 0:
        point_id = sys.argv[1]
        r.get(point_id)
        val = r.get(point_id)['label']
        print(val)
    
    if (len(sys.argv) > 1 and val):
        s = Simulator()
        s.gen(sys.argv[1], val)