...
func (l GroupDevList) String() string {
	b, _ := json.Marshal(l)
	return string(b)
}
...
func GetGroupDev() (GroupDevList, error) {

	req, err := http.NewRequest(http.MethodGet, fmt.Sprintf("%s/device_template/groups_dev.json", auth.GetAddr()), nil)
	if err != nil {
		return nil, err
	}
